const game = () => {
    ...
    function setDifficulty(difficulty) {//set game difficulty
        switch (difficulty) {
    ...
    function startGame() { //when the start button is clicked
        ...
        for (let i = 0; i < 5; i++) {//repeat for each row in bingo board
            let columnNumbers = [];
            let j = 0;
            while (j < 5) {//repeat for each column in each row
                let cellNumber = Math.floor(Math.random() * 15) + i * 15 + 1;//each column has 15 numbers
                if (!columnNumbers.includes(cellNumber)) {//check if other numbers are not in bingo board
        ...
        if (freeSpaceButton.checked) {//check if the free space button is checked
            cellsMarked[2][2] = true;//mark the center cell on the free space on the bingo board
            bingoTable.querySelector("#row-3").querySelector("#cell-3-3").innerText = "";//update the text
        }
        for (let i = 0; i < 75; i++) {
            numbersRemaining.push(i + 1);//add numbers to numbers remaining
        }
        for (let i = 0; i < numbersRemaining.length; i++) {//repeat for each number for numbers remaining array
            let randomIndex = Math.floor(Math.random() * numbersRemaining.length);
            [numbersRemaining[i], numbersRemaining[randomIndex]] = [numbersRemaining[randomIndex], numbersRemaining[i]];//shuffle the numbers remaining array
        }
        if (difficultyMode !== "speed-mode") {//if difficulty mode is not speed mode
    ...
    function callNumbers() {
        if (isGameStarted && !isGameEnded) {
            if (numbersLeft > 0) {//if there are numbers left to call
    ...
    function skipNumbers() {//do action when skip number button is pressed
        if (isGameStarted && !isGameEnded) {//check if game is started and game is not ended
            if (difficultyMode === "speed-mode") {//if game difficulty mode is speed mode
                if (!bingoBoard.includes(numbersCalled[0])) {//if last number called is not in bingo board
                ...
                else {//if last number called is in bingo board
                    streak = 0;//reset streak to 0
                }
            }
        }
    }
    function increaseScore(amount) {//increase score based on various multipliers
        score += Math.round(amount * comboMultiplier * bingoMultiplier * timeMultiplier * (1 + streak / 10) * calledBonus * (1 + timeCombo / 10) * difficultyMultiplier * bingosMadeMultiplier * freeSpaceMultiplier);//round the score to nearest integer
    }
    function increaseScoreBingo(bingoNumbersCalled) {//increase score based on the number of bingo numbers called
        score = Math.round(score * 75 / bingoNumbersCalled);//round the score to nearest integer
        scoreText.innerText = score.toLocaleString("en-US");//update score text with commas as a thousand separator
    }
    function markCell(y, x) {//mark cell on specified cell position
        if (isGameStarted && !isGameEnded) {//check if the game is started and not ended
            if (numbersCalled.includes(bingoBoard[x - 1][y - 1]) && cellsMarked[x - 1][y - 1] === undefined) {//check if numbers called are part of bingo board and cell is not marked
                if (numbersCalled[0] === bingoBoard[x - 1][y - 1]) {//check if the current number called is in the bingo board
                    if (difficultyMode !== "speed-mode") {//if difficulty mode is not speed mode
                    ...
                    if (((lastCalled === numbersCalled[1]) || (lastCalled === undefined && numbersCalled.length === 1)) && difficultyMode !== "speed-mode") {//check if the last marked number is marked or if cell marked on first number called and game difficulty is not speed mode
                ...
                if (difficultyMode === "speed-mode") {//check if difficulty mode is speed mode
                    timeMultiplier = 1 + (1000 / (performance.now() - timeCalled + 1000));//double the time multiplier
                }
                else {
                    timeMultiplier = 1 + (timeBonus / (performance.now() - timeCalled + timeBonus));//set time bonus multiplier based on how fast the user mark the cell
                }
                if (calledBonus === 2) {
                ...
                for (let row = 0; row < 5; row++) {//repeat for each row
                    for (let column = 0; column < 4; column++) {//check for 2 in a row
                        if (cellsMarked[row][column] === true && cellsMarked[row][column + 1] === true) {//check for each column
                            if (lastCalled === bingoBoard[row][column] || lastCalled === bingoBoard[row][column + 1]) {//check if the last called number is one of the cells marked
                        ...
                        if (cellsMarked[column][row] === true && cellsMarked[column + 1][row] === true) {//check for each row
                        ...
                        if (cellsMarked[column][column] === true && cellsMarked[column + 1][column + 1] === true) {//check for top-left to bottom-right diagonal
                        ...
                        if (cellsMarked[column][4 - column] === true && cellsMarked[column + 1][3 - column] === true) {//check for top-right to bottom-left diagonal
                    ...
                    for (let column = 0; column < 3; column++) {//check for 3 in a row
                    ...
                    for (let column = 0; column < 2; column++) {//check for 4 in a row
                ...
                bingoTable.querySelector(`#row-${y}`).querySelector(`#cell-${y}-${x}`).innerText = "";//mark the cell on the bingo board
                ...
                if (cellsMarked.every(row => row.every(cell => cell !== undefined)) && (isGameStarted && !isGameEnded)) {//if all cells are marked and game is started and game is not ended, end the game
                    isGameStarted = false;//set is game started to false
                    isGameEnded = true;//set is game ended to true
                    startButton.disabled = false;//enable the start button
                    easyButton.disabled = false;//enable easy difficulty button
            ...
            else if (cellsMarked[x - 1][y - 1] === undefined) {//reset streak and combo if cell is clicked when number is not called yet
                streak = 0;
                combo = 0;
            }
        }
    ...
    function checkBingos() {
        ...
        for (let column = 0; column < 5; column++) {//repeat for each column
        ...
        while (bingoCount < bingos) {//loop for bingo counts to make sure to increase score once for each bingo
            bingoCount++;
            switch (bingoCount) {
                case 1:
