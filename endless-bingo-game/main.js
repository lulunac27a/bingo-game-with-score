const game = () => {
    ...
    let time180SecondsButton = document.getElementById("180-seconds"); //180-second time button
    let time300SecondsButton = document.getElementById("300-seconds"); //300-second time button
    ...
    const skipNumbers = () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (bingoBoard[i].includes(lastCalled)) {
                //if the last number called is in the bingo board
    ...
    const markCell = (y, x) => {
        ...
        if (isGameStarted && !isGameEnded) {
            ...
            if (
                ...
                //check if the number is part of bingo board and the cell is not marked
                ...
                streak++; //increase streak by 1
                timeMultiplier =
                    1 + 1000 / (performance.now() - timeCalled + 1000); //set time multiplier based on how fast the cell is clicked correctly
                timeCalled = performance.now(); //set time called to current performance time
    ...
    const callNumbers = () => {
        ...
        while (!sameNumber) {
            //check if the next number called is different from the last called number
    ...
    const generateBingoNumber = (x, y) => {
        ...
        while (i < 5) {
            ...
            if (bingoBoard[x][i] === cellNumber) {
                //if new bingo number is the same as bingo number of bingo board
                i = 0; //reset iteration to 0
                cellNumber = Math.floor(Math.random() * 15) + x * 15 + 1; //set the new bingo number based on cell position
